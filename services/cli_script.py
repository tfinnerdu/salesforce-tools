...
def _readme(project: str, fields: list, permset_name: str, alias: str,
            extra_permset_names: list = None, object_names: list = None) -> str:
    lines = [
    ...
    lines += ['']
    if object_names:
        # A member (-m) deploy can't create the object + its fields in one pass
        # reliably; the whole-folder deploy is the dependable path for a clone.
        lines += [
            'Deploy the whole folder (creates the object, then its fields):',
...
def build_package_zip(project: str, fields: list, permset: dict = None,
                      ...
                      extra_permsets: list = None, object_shells: list = None) -> tuple:
    ...
    if not fields and not all_permsets and not object_shells:
        raise ValueError('Nothing to package — add at least one field, a permission set, or an object.')
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr('manifest/package.xml',
                    package_xml(fields, permset_name, api_version, extra_names, object_names))
        zf.writestr('README.txt',
                    _readme(project, fields, permset_name, alias, extra_names, object_names))
