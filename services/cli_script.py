...
def package_xml(fields: list, permset_name: str = '',
    ...
    if permset_name:
        lines.append('    <types>')
        lines.append(f'        <members>{escape(permset_name)}</members>')
...
def _members(fields: list, permset_name: str) -> list:
...
def deploy_snippet(fields: list, permset_name: str, alias: str,
    ...
    alias = alias or '<alias>'
    members = _members(fields, permset_name)
...
def _readme(project: str, fields: list, permset_name: str, alias: str) -> str:
    lines = [
    ...
    lines += [
        ...
        '  ' + deploy_snippet(fields, permset_name, alias, dry_run=True).replace('\n', '\n  '),
        '',
        'Then commit:',
        '  ' + deploy_snippet(fields, permset_name, alias, dry_run=False).replace('\n', '\n  '),
    ]
    return '\n'.join(lines) + '\n'
...
def build_package_zip(project: str, fields: list, permset: dict = None,
                      alias: str = '', api_version: str = DEFAULT_API_VERSION) -> tuple:
    ...
    Returns (zip_bytes, filename). Layout:
      ...
      README.txt
    """
    if not fields and not (permset and permset.get('field_perms')):
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        if permset_name:
            path = f'force-app/main/default/permissionsets/{permset_name}.permissionset-meta.xml'
            zf.writestr(path, permission_set_xml(
                permset_name, permset.get('label', permset_name),
                permset.get('field_perms', []), permset.get('description', '')))
        zf.writestr('manifest/package.xml', package_xml(fields, permset_name, api_version))
        zf.writestr('README.txt', _readme(project, fields, permset_name, alias))
