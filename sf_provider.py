"""Salesforce provider — live simple_salesforce client by default; a
MockSalesforce when SHOW_MOCK=true.

There is no silent mock fallback. With SHOW_MOCK=false (the default) an
unconfigured org makes get_sf() raise RuntimeError instead of fabricating
data. SHOW_MOCK=true returns a MockSalesforce for any org and is intended
for manual UI/demo testing — the unit-test suite uses unittest.mock
...
def org_is_mocked(org: str) -> bool:
    ...
    """
    return Config.SHOW_MOCK or not _configured(org)
...
def assert_orgs_comparable(left_org: str, right_org: str) -> None:
    ...
    With SHOW_MOCK on, both sides resolve to the same mock — trivially
    comparable. With SHOW_MOCK off, both orgs must have credentials configured;
    diffing against an unconfigured org would otherwise fail mid-run.
    """
    if Config.SHOW_MOCK:
        return
...
def get_sf(org: str = 'dev'):
    """Return a Salesforce client for the given org.

    With SHOW_MOCK=true, returns a MockSalesforce. Otherwise returns a live
    simple_salesforce client; raises RuntimeError when the org has no
    credentials configured (no silent mock fallback).
    """
    if Config.SHOW_MOCK:
        logger.info("SHOW_MOCK active — using MockSalesforce for org '%s'", org)
    ...
    if not _configured(org):
        raise RuntimeError(
            ...
            f"or enable SHOW_MOCK=true for the demo layer."
        )
