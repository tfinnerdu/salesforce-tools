...
def test_import_fields_returns_field_list(client):
    resp = client.post('/api/v1/data-ops/import/fields', json={'object': 'Account'})
    assert resp.status_code == 200
    body = resp.get_json()
...
def test_import_fields_missing_object_returns_400(client):
    resp = client.post('/api/v1/data-ops/import/fields', json={})
...
def test_import_validate_returns_summary(client):
    resp = client.post('/api/v1/data-ops/import/validate', data={
        'object': 'Account',
        'operation': 'insert',
...
def test_import_execute_returns_results(client):
    resp = client.post('/api/v1/data-ops/import/execute', data={
        'object': 'Account',
        'operation': 'insert',
        'field_mapping': json.dumps({'Name': 'Name', 'SIS_ID__c': 'SIS_ID__c'}),
        'csv_file': _csv_upload(),
...
def test_import_download_errors_returns_csv(client):
    resp = client.post('/api/v1/data-ops/import/download-errors', data={
...
def test_delete_preview_returns_rows(client):
    resp = client.post('/api/v1/data-ops/delete/preview',
...
def test_delete_preview_missing_params_returns_400(client):
    resp = client.post('/api/v1/data-ops/delete/preview', json={'object': 'Account'})
    assert resp.status_code == 400


def test_delete_execute_returns_counts(client):
    resp = client.post('/api/v1/data-ops/delete/execute',
...
def test_modify_preview_returns_rows(client):
    resp = client.post('/api/v1/data-ops/modify/preview',
...
def test_modify_execute_returns_counts(client):
    resp = client.post('/api/v1/data-ops/modify/execute', json={
...
def test_modify_execute_missing_updates_returns_400(client):
    resp = client.post('/api/v1/data-ops/modify/execute',
                       json={'object': 'Account', 'where_clause': 'Id != null'})
    assert resp.status_code == 400
...
def test_reassign_users_search(client):
    resp = client.get('/api/v1/data-ops/reassign/users?q=')
    assert resp.status_code == 200
    assert resp.get_json()['success'] is True


def test_reassign_preview_returns_rows(client):
    resp = client.post('/api/v1/data-ops/reassign/preview',
                       json={'object': 'Account', 'where_clause': 'Id != null'})
    assert resp.status_code == 200
    assert resp.get_json()['success'] is True
...
def test_reassign_execute_returns_counts(client):
    resp = client.post('/api/v1/data-ops/reassign/execute', json={
        'object': 'Account', 'where_clause': 'Id != null',
...
def test_reassign_execute_missing_owner_returns_400(client):
    resp = client.post('/api/v1/data-ops/reassign/execute',
                       json={'object': 'Account', 'where_clause': 'Id != null'})
    assert resp.status_code == 400
...
def test_export_run_accepts_form_post(client):
    resp = client.post('/api/v1/data-ops/export/run', data={
...
def test_export_run_missing_soql_returns_400(client):
    resp = client.post('/api/v1/data-ops/export/run', json={})
    assert resp.status_code == 400
...
@pytest.mark.parametrize('path,payload,module,func', [
    ('/api/v1/data-ops/import/fields',     {'object': 'Account'},
     'services.data_importer', 'get_object_fields'),
    ('/api/v1/data-ops/delete/preview',    {'object': 'Account', 'where_clause': 'Id != null'},
     'services.bulk_ops', 'bulk_delete_preview'),
    ('/api/v1/data-ops/delete/execute',    {'object': 'Account', 'where_clause': 'Id != null'},
     'services.bulk_ops', 'bulk_delete_execute'),
    ('/api/v1/data-ops/modify/preview',    {'object': 'Account', 'where_clause': 'Id != null', 'field': 'Name'},
     'services.bulk_ops', 'bulk_modify_preview'),
    ('/api/v1/data-ops/modify/execute',    {'object': 'Account', 'where_clause': 'Id != null',
                                     'field_updates': {'Name': 'X'}},
     'services.bulk_ops', 'bulk_modify_execute'),
    ('/api/v1/data-ops/reassign/preview',  {'object': 'Account', 'where_clause': 'Id != null'},
     'services.bulk_ops', 'bulk_reassign_preview'),
    ('/api/v1/data-ops/reassign/execute',  {'object': 'Account', 'where_clause': 'Id != null',
                                     'new_owner_id': '005000000000000001'},
     'services.bulk_ops', 'bulk_reassign_execute'),
    ('/api/v1/data-ops/export/run',        {'soql': 'SELECT Id FROM Account'},
...
def test_reassign_users_error_returns_500(client, monkeypatch):
    ...
    resp = client.get('/api/v1/data-ops/reassign/users?q=x')
    assert resp.status_code == 500
    assert resp.get_json()['success'] is False
...
# ─────────────────────────────────────────────────────────────────────────────
# Legacy JSON path redirect — old /data-ops/<subpath> JSON calls 308-redirect
# to their new home under /api/v1/data-ops (register_legacy_json_redirect),
# mirroring TestMetaLegacyRedirect in tests/test_meta_routes.py.
# ─────────────────────────────────────────────────────────────────────────────

class TestDataOpsLegacyRedirect:
    def test_old_path_redirects_to_versioned_path(self, client):
        resp = client.get('/data-ops/tune/rules', follow_redirects=False)
        assert resp.status_code == 308
        assert resp.headers['Location'].endswith('/api/v1/data-ops/tune/rules')

    def test_old_path_redirect_is_followable(self, client):
        resp = client.get('/data-ops/tune/rules', follow_redirects=True)
        assert resp.status_code == 200
        assert resp.get_json()['success'] is True
