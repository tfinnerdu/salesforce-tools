...
preview/apply pipeline against SF doubles, and the routes.
"""
import pytest
...
class _StubSF:
    """Covers preview_tune: query() for sample + COUNT()."""
...
class _LiveSF:
    """Covers apply_tune: query_all() for the id set + a bulk2 update stub."""
...
def test_apply_tune_computes_real_counts(monkeypatch):
    """The query+transform runs for real; the bulk write is exercised via a stub."""
    records = [
        {'Id': '001', 'Name': 'JOHN SMITH'},
        {'Id': '002', 'Name': 'BOB JONES'},
        {'Id': '003', 'Name': 'Jane Doe'},     # already clean
    ]
    sf = _LiveSF(records, processed=2, failed=0)
...
def test_apply_tune_no_changes_returns_zero(monkeypatch):
    ...
    monkeypatch.setattr(data_tuner, 'get_sf',
                        lambda org: _LiveSF(records, processed=0, failed=0))
    result = data_tuner.apply_tune('dev', 'Account', 'Id != null', {'Name': ['proper_case']})
...
def test_apply_tune_bypass_triggers_toggled(monkeypatch):
...
def test_apply_tune_writes_only_changed_fields(monkeypatch):
    records = [
...
def test_tune_execute_route(client, monkeypatch):
    monkeypatch.setattr(data_tuner, 'get_sf',
