...
get_record_counts issues one COUNT() query per tracked object via a
`unittest.mock` Salesforce double.
"""
import pytest
...
def test_get_record_counts_returns_list():
    ...
    with patch('services.storage_breakdown.get_sf', return_value=_make_sf_mock()):
...
def test_get_record_counts_uses_mocked_sf():
    from services.storage_breakdown import get_record_counts
