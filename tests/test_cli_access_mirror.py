...
def _run_plan(cloned_fields=None, describe=None):
    src = _source_sf(
        [_op(True, 'System Administrator', read=True, edit=True),
         _op(False, 'Housing_Staff', read=True),
         _op(False, 'Ghost_Set', read=True)],
        [_fp(True, 'System Administrator', 'RoomAssignment__c.Room__c', read=True, edit=True),
         _fp(True, 'System Administrator', 'RoomAssignment__c.Legacy__c', read=True),
         _fp(False, 'Housing_Staff', 'RoomAssignment__c.Room__c', read=True)])
    tgt = _target_sf(['System Administrator'], {'Housing_Staff': 'Housing Staff'},
                     describe=describe)

    def fake(org):
        return src if org == 'eda' else tgt
    with patch('services.cli_access_mirror.get_sf', side_effect=fake):
        return mirror.mirror_plan('eda', 'sandbox', 'RoomAssignment__c', cloned_fields)
...
def test_mirror_matches_by_name():
    ...
    assert matched == {'System Administrator', 'Housing_Staff'}
...
def test_route_access_mirror_plan(client):
    src = _source_sf([_op(True, 'System Administrator', read=True)],
...
def test_clone_package_with_mirror(client):
    ...
    mirror_src = _source_sf(
        [_op(True, 'System Administrator', read=True, edit=True),
         _op(False, 'Housing_Staff', read=True)],
        [_fp(True, 'System Administrator', 'Accommodation__c.Notes__c', read=True, edit=True)])
...
def test_clone_plan_includes_mirror_preview(client):
    src = _clone_sf()
    mirror_src = _source_sf([_op(True, 'System Administrator', read=True)],
                            [_fp(True, 'System Administrator', 'Accommodation__c.Notes__c', read=True)])
    tgt = _target_sf(['System Administrator'], {}, describe=None)
