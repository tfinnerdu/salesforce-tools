import logging

from flask import Blueprint, render_template, request, session

from services import org_observer, storage_breakdown, sandbox_drift
from utils.responses import error_response, new_request_id, ok, register_legacy_json_redirect

logger = logging.getLogger(__name__)

# HTML page — stays unversioned at the existing prefix, per the Doane
# standard. JSON data/action routes moved to observe_api_bp under
# /api/v1/observe; the old /observe/<subpath> paths 308-redirect there
# (register_legacy_json_redirect below) so existing MC.api('/observe/...')
# calls keep working unmodified.
observe_bp = Blueprint('observe', __name__, url_prefix='/observe')
observe_api_bp = Blueprint('observe_api', __name__, url_prefix='/api/v1/observe')


@observe_api_bp.before_request
def _assign_request_id():
    from flask import g
    g.request_id = new_request_id()


# ── Page routes ───────────────────────────────────────────────────────────────

@observe_bp.route('/')
@observe_bp.route('')
def index():
    return render_template('observe/index.html')


register_legacy_json_redirect(observe_bp, '/api/v1/observe')


# ── API routes ────────────────────────────────────────────────────────────────

@observe_api_bp.route('/limits')
def api_limits():
    org = request.args.get('org') or session.get('active_org', 'dev')
    try:
        data = org_observer.get_limits(org)
        return ok(data)
    except Exception as exc:
        logger.exception('get_limits failed for org %s', org)
        return error_response(str(exc), 'LIMITS_FAILED', 500)


@observe_api_bp.route('/trends')
def api_trends():
    org = request.args.get('org') or session.get('active_org', 'dev')
    try:
        days = int(request.args.get('days', 30))
    except (ValueError, TypeError):
        days = 30
    try:
        data = org_observer.get_quality_trends(org, days=days)
        return ok(data)
    except Exception as exc:
        logger.exception('get_quality_trends failed for org %s', org)
        return error_response(str(exc), 'TRENDS_FAILED', 500)


@observe_api_bp.route('/record-counts')
def api_record_counts():
    org = request.args.get('org') or session.get('active_org', 'dev')
    try:
        data = storage_breakdown.get_record_counts(org)
        return ok(data)
    except Exception as exc:
        logger.exception('get_record_counts failed for org %s', org)
        return error_response(str(exc), 'RECORD_COUNTS_FAILED', 500)


@observe_api_bp.route('/sandbox-drift')
def api_sandbox_drift():
    baseline = request.args.get('baseline', 'prod')
    target = request.args.get('target', 'sandbox')
    try:
        data = sandbox_drift.run(baseline_org=baseline, target_org=target)
        return ok(data)
    except Exception as exc:
        logger.exception('sandbox drift failed')
        return error_response(str(exc), 'SANDBOX_DRIFT_FAILED', 500)


@observe_api_bp.route('/cross-org')
def api_cross_org():
    orgs_raw = request.args.get('orgs', 'dev,sandbox')
    orgs = [o.strip() for o in orgs_raw.split(',') if o.strip()]
    if not orgs:
        orgs = ['dev', 'sandbox']
    try:
        data = org_observer.get_cross_org_counts(orgs)
        return ok(data)
    except Exception as exc:
        logger.exception('get_cross_org_counts failed for orgs %s', orgs)
        return error_response(str(exc), 'CROSS_ORG_FAILED', 500)


@observe_api_bp.route('/thresholds', methods=['GET'])
def api_get_thresholds():
    try:
        data = org_observer.get_thresholds()
        return ok(data)
    except Exception as exc:
        logger.exception('get thresholds failed')
        return error_response(str(exc), 'GET_THRESHOLDS_FAILED', 500)


@observe_api_bp.route('/thresholds', methods=['POST'])
def api_set_threshold():
    body = request.get_json(silent=True) or {}
    limit_name = body.get('limit_name', '').strip()
    try:
        amber_pct = float(body.get('amber_pct', 50))
        red_pct   = float(body.get('red_pct', 80))
    except (TypeError, ValueError):
        return error_response('amber_pct and red_pct must be numbers', 'INVALID_INPUT', 400)
    if not limit_name:
        return error_response('limit_name required', 'INVALID_INPUT', 400)
    if not (0 < amber_pct < red_pct <= 100):
        return error_response('amber_pct must be < red_pct and both 0–100', 'INVALID_INPUT', 400)
    try:
        data = org_observer.set_threshold(limit_name=limit_name, amber_pct=amber_pct, red_pct=red_pct)
        return ok(data)
    except Exception as exc:
        logger.exception('set threshold failed')
        return error_response(str(exc), 'SET_THRESHOLD_FAILED', 500)


@observe_api_bp.route('/thresholds/<limit_name>', methods=['DELETE'])
def api_delete_threshold(limit_name):
    try:
        deleted = org_observer.delete_threshold(limit_name)
        return ok({'deleted': deleted})
    except Exception as exc:
        logger.exception('delete threshold failed')
        return error_response(str(exc), 'DELETE_THRESHOLD_FAILED', 500)
