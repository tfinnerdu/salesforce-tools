"""Conductor provider — live ConductorClient by default; a
MockConductorClient when SHOW_MOCK=true.

There is no silent mock fallback. With SHOW_MOCK=false (the default) a
missing CONDUCTOR_URL/CONDUCTOR_API_KEY makes get_conductor_client() raise
RuntimeError instead of fabricating workflow data. SHOW_MOCK=true returns a
MockConductorClient and is intended for manual UI/demo testing.
"""
import logging
import os
import time
...
def get_conductor_client():
    """Return a ConductorClient for the configured Conductor instance.

    With SHOW_MOCK=true, returns a MockConductorClient. Otherwise returns a
    live ConductorClient; raises RuntimeError when Conductor is not configured
    (no silent mock fallback).
    """
    if Config.SHOW_MOCK:
        logger.info("SHOW_MOCK active — using MockConductorClient")
        return MockConductorClient()
    if not _configured():
        raise RuntimeError(
            "Conductor is not configured. Set CONDUCTOR_URL and "
            "CONDUCTOR_API_KEY in the environment, or enable SHOW_MOCK=true "
            "for the demo layer."
        )
